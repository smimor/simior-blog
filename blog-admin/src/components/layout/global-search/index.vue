...
const handleKeydown = (event: KeyboardEvent) => {
  ...
  // 使用 userAgent 来检测是否为 Mac 系统（更可靠的现代方法）
...
const search = (val: string) => {
  if (val && val.trim()) {
    searchResult.value = flattenAndFilterMenuItems(menuList.value, val.trim())
  } else {
    searchResult.value = []
  }
}
...
const selectHighlighted = () => {
  if (searchVal.value && searchResult.value.length) {
    searchGoPage(searchResult.value[highlightedIndex.value] as AppRouteRecord)
  } else if (!searchVal.value && historyResult.value.length) {
    searchGoPage(historyResult.value[historyHIndex.value] as AppRouteRecord)
  }
}
  ...
  <div class="layout-search">
    ...
    >
      ...
      >
        <template #suffix>
          <div
            class="h-4.5 flex justify-center items-center rounded border border-g-300 dark:bg-g-200/50! bg-box! px-1.5! text-g-500"
          >
            <SvgIcon icon="fluent:arrow-enter-left-20-filled" />
          </div>
        </template>
      </el-input>
      <el-scrollbar class="mt-5!" max-height="370px" ref="searchResultScrollbar" always>
        <div class="result w-full" v-show="searchResult.length">
          ...
          >
            <div
              class="mt-2! h-12 flex items-center justify-between rounded-lg bg-g-200/80 px-4! text-sm text-g-700"
      ...
      <template #footer>
        <div class="dialog-footer">
          <div class="dialog-item">
            ...
            <span class="item-label">{{ $t('search.selectKeydown') }}</span>
          </div>
          <div class="dialog-item">
            <SvgIcon icon="ri:arrow-up-wide-fill" class="keyboard" />
            <SvgIcon icon="ri:arrow-down-wide-fill" class="keyboard" />
            <span class="item-label">{{ $t('search.switchKeydown') }}</span>
          </div>
          <div class="dialog-item">
            <i class="keyboard w-8! flex items-center justify-center"><p class="text-[10px] font-medium">ESC</p></i>
            <span class="item-label">{{ $t('search.exitKeydown') }}</span>
          </div>
        </div>
      </template>
    </el-dialog>
