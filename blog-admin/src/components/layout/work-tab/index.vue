...
import { useSettingStore, useUserStore, useWorkTabStore } from '@/stores'
import { formatMenuTitle } from '@/utils/router'
import type { MenuItemType } from '@/components/other/menu-right/index.vue'
...
<template>
  <div v-if="showWorkTab" class="work-tab max-sm:px-3.75!">
    ...
    <div class="flex">
      <div
        class="arrow-down art-card-xs size-8 leading-8 tad-200"
        :style="{
          borderRadius: 'calc(var(--custom-radius) / 2.5 + 0px)'
        }"
        @click="(e: MouseEvent) => showMenu(e, activeTab)"
      >
        <SvgIcon icon="iconamoon:arrow-down-2-thin" class="text-2xl text-g-700" />
      </div>
    </div>
